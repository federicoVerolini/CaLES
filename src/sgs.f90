    ...
    if(is_bound(0,1).and.cbc(0,1,1)=='D') then
      do k=1,n(3)
        do j=1,n(2)
          ...
          w_icm = w(1, j   ,k-1)
          w_ocm = w(0, j   ,k-1)
          tauw(1) = visc*0.5_rp*(v_icc-v_occ+v_imc-v_omc)/dl(1)
          tauw(2) = visc*0.5_rp*(w_icc-w_occ+w_icm-w_ocm)/dl(1)
    ...
    if(is_bound(0,2).and.cbc(0,2,2)=='D') then
      do k=1,n(3)
        do i=1,n(1)
          ...
          w_cim = w(i  ,1,k-1)
          w_com = w(i  ,0,k-1)
          tauw(1) = visc*0.5_rp*(u_cic-u_coc+u_mic-u_moc)/dl(2)
          tauw(2) = visc*0.5_rp*(w_cic-w_coc+w_cim-w_com)/dl(2)
          tauw_tot= sqrt(tauw(1)*tauw(1) + tauw(2)*tauw(2))
          delv    = visc/sqrt(tauw_tot)
          do j = 1,n(2)
            this_dw = dl(2)*(j-0.5)
    ...
    if(is_bound(0,3).and.cbc(0,3,3)=='D') then
      do j=1,n(2)
        do i=1,n(1)
          ...
          v_cmi = v(i  ,j-1,1)
          v_cmo = v(i  ,j-1,0)
          tauw(1) = visc*0.5_rp*(u_cci-u_cco+u_mci-u_mco)/dzc(0)
          tauw(2) = visc*0.5_rp*(v_cci-v_cco+v_cmi-v_cmo)/dzc(0)
          tauw_tot= sqrt(tauw(1)*tauw(1) + tauw(2)*tauw(2))
          delv    = visc/sqrt(tauw_tot)
          do k = 1,n(3)
            this_dw = zc(k)
    ...
    if(is_bound(1,3).and.cbc(1,3,3)=='D') then
      do j=1,n(2)
        do i=1,n(1)
          ...
          v_cmi = v(i  ,j-1,n(3)  )
          v_cmo = v(i  ,j-1,n(3)+1)
          tauw(1) = visc*0.5_rp*(u_cci-u_cco+u_mci-u_mco)/dzc(n(3))
          tauw(2) = visc*0.5_rp*(v_cci-v_cco+v_cmi-v_cmo)/dzc(n(3))
          tauw_tot= sqrt(tauw(1)*tauw(1) + tauw(2)*tauw(2))
          delv    = visc/sqrt(tauw_tot)
          do k = 1,n(3)
            this_dw = l(3)-zc(k)
